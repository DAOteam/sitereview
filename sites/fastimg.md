...
- 当前问题与线上证据：2026-09-24 在线点击 `Generate from text` 后，页面立即创建一张标为 `Blank canvas` 的黑色空白图片，在时间线新增编号 0 的 `Blank canvas` 版本，并启用 Download；浏览器焦点仍停留在页面根节点，没有进入提示词输入框。用户尚未输入或生成任何内容，却先看到一个可下载的虚假作品版本，且还要再次找到输入框才能开始文生图。
...
- 当前问题与线上证据：2026-09-24 在 1280×720 视口进入空白画布后，当前作品预览位于 x=391–889、y=115–488，而 Download 位于页面顶栏 x=1152–1258、y=10–46；按钮与作品右边缘横向相隔约 263px，并与作品所在的编辑上下文分离。用户在查看或切换时间线版本后，需要把视线和指针移动到页面右上角才能下载，且按钮的位置不能直观说明将下载哪个版本。
...
- 当前问题与线上证据：2026-09-24 在 1280px 宽视口实测 Compare 时，比较容器和 before 图片的边界均为 x=26、y=126、宽 1228px、高约 362.3px，但 after 图片边界为 x=26、y=126、宽 1280px、高约 362.3px；after 比容器和 before 宽 52px。两张测试图片的原始尺寸同为 960×720，且均使用 `object-fit: contain` 和 `object-position: 50% 50%`，因此可见错位来自 after 图层错误地按视口宽度布局，而不是图片尺寸或生成内容差异。滑块改变 after 的可见区域时，两层不能按相同坐标完全重合。
...
- 当前问题与线上证据：2026-09-24 在 320×568 视口实测时，页面 `documentElement.clientWidth` 为 320px，但 `documentElement.scrollWidth` 和 `body.scrollWidth` 均为 347px。底部提示词输入框占据 42–228px，Generate 按钮占据 251–347px，按钮右侧 27px 超出视口；54×54px 的客服悬浮按钮位于 246–300px、494–548px，同时覆盖提示词输入区和 Generate 按钮。375×667 和 390×844 视口虽然未产生页面级横向溢出，客服按钮仍覆盖 Generate 按钮，导致核心提交操作难以点击。顶部 Download、首屏 Choose image、Generate from text 和底部 Generate 的可见高度均为 36px，也低于移动端 44px 触控目标。
...
- 优先级：`P2`
- 页面或界面：https://fastimg.ai/、https://fastimg.ai/product-photo-editor/
- 当前问题与线上证据：英语首页 H1 视觉上分为两行，但浏览器 DOM 的 `textContent` 实际是 `Edit images by typing,a free AI image editor`，逗号后缺少空格；产品图页的操作说明 H2 当前是语法错误且大小写错误的 `How to aI product photo editor`。其他五个语言首页的 H1 以及已本地化 H2、图片 alt 和 WebApplication `featureList` 已经在线正确，不再需要修改。
- 修改要求：将英语首页 H1 的 DOM 纯文本修正为 `Edit images by typing, a free AI image editor`，在逗号后输出一个真实空格字符，同时保留当前两行视觉布局。将产品图页 H2 `How to aI product photo editor` 逐字替换为 `How to edit product photos with AI`。
- 验收标准：在启用和禁用 JavaScript 两种状态下，英语首页只有一个 H1，`document.querySelector('h1').textContent` 逐字等于 `Edit images by typing, a free AI image editor`，桌面端和 390px 移动端仍保持现有两行排版且无横向滚动。产品图页只出现一个 `How to edit product photos with AI` H2，初始 HTML、DOM、可访问性树和纯文本提取中都不再出现 `How to aI product photo editor`。两页的 Title、Meta Description、Canonical、robots 和 JSON-LD 保持不变。
- 不要修改：不要改写英语首页或产品图页的其他文案，不要更换首页 H1 或产品图页 H2 的意图，不要改动任何已正确的多语言标题、图片 alt、结构化数据、内链、工具入口或页面布局。

### 发布移动端编辑器兼容性更新的公开变更日志

- 优先级：`P1`
- 页面或界面：https://fastimg.ai/changelog/
- 当前问题与线上证据：`/app/` 当前在 320px 宽度下出现页面级横向溢出，并在 320px、375px 和 390px 宽度下被客服悬浮按钮遮挡核心提交操作。本批移动端布局修复属于重要的用户可见兼容性更新；现有公开 Changelog 已建立，但尚未包含这项仍未上线的改进。
- 修改要求：仅在“修复 App 在手机窄屏下的横向溢出与操作遮挡”任务通过生产环境验收后，使用真实发布日期在现有 Changelog 顶部新增标题 `Mobile editor improvements`，正文逐字使用 `The FastImg editor now works smoothly on small screens. The prompt field, Generate button, and support chat stay accessible without overlap, with larger touch targets and safe-area spacing on mobile devices.`。保持现有条目、日期、页面 Title、Meta Description、Canonical 和结构化数据不变，并同步该页面在 XML Sitemap 中的真实 `lastmod`。
- 验收标准：移动端修复已在生产环境通过该任务规定的全部视口与状态测试后，Changelog 才显示使用实际上线日期的新条目；标题和正文与指定文案逐字一致，条目位于旧条目之前。页面返回 200、允许索引、自引用 Canonical，XML Sitemap 中的 Changelog `lastmod` 与该条目发布日期一致；条目描述的输入区、Generate、客服、触控目标和安全区改进均可在线复现。
- 不要修改：不要在移动端修复上线前发布条目，不要预填或虚构发布日期，不要改写或删除已有历史条目与日期；不要在公开变更日志中提及文件、组件、CSS 类、代码、仓库、分支、提交、部署、测试设备、内部指标或客服供应商。
