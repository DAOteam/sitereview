...
title: "Improve workspace navigation, result actions, and mobile layout"
...
A third user-provided desktop screenshot dated 2026-08-09 shows that the primary navigation begins with `How it works` and has no explicit `Workspace` item. The user requests a `Workspace` item immediately before `How it works`, linking to the homepage.
...
4. Add `Workspace` to the top navigation immediately before `How it works`, and link it to the homepage.
...
- Add a visible `Workspace` link as the first item in the English primary navigation, immediately before `How it works`.
- Link `Workspace` to the English homepage root (`/`). Do not create a new route or point it to a separate authenticated workspace URL.
...
- Apply the same item and ordering to the mobile navigation or menu: `Workspace`, then `How it works`, followed by the existing items.
...
4. Localization scope for the new navigation item. AI recommendation: add `Workspace` only to the English navigation in this task. Do not insert an untranslated English label into Spanish, Portuguese, French, or German pages; localize those menus only when the user authorizes the separate multilingual copy pass.
...
- Provisional: the English desktop primary navigation contains `Workspace` immediately before `How it works`, and the link target is `/`.
- Provisional: the English mobile navigation or menu contains the same `Workspace` item in the same relative order.
...
- Provisional: adding `Workspace` causes no desktop navigation overflow, clipping, illegible compression, or collision at supported widths.
...
- Creating a new Workspace route, changing authenticated routing, or changing where the homepage tool runs.
