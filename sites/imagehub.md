...
### 将 Workspace 工具选择器的 No tool 改为 Auto

- 优先级：P2
- 页面或界面：首页集合 Workspace、登录后的完整 Workspace，以及桌面端和移动端任务创建模块中的工具选择按钮、Choose a tool 下拉菜单和当前选中状态
- 当前问题与线上证据：用户于 2026-09-15 提供的截图显示，Choose a tool 下拉菜单第一项和当前默认项使用 No tool，并说明会根据提示词及是否附加图片选择 Text to Image 或 Image to Image。No tool 容易被理解为没有功能或不能处理任务，不能准确表达系统正在自动选择基础生成模式。
- 修改要求：将工具选择器第一项、默认选中项以及选中后显示在原 Tools 按钮位置的 No tool 或 No tools 全部统一改为精确文案 Auto。该选项下方说明统一改为精确文案 Automatically uses Text to Image, or Image to Image when you attach an image.。Auto 继续排在下拉菜单第一项并保持默认选中；没有附加图片时继续使用现有 Text to Image 流程，附加至少一张图片后继续使用现有 Image to Image 流程，移除全部图片后恢复 Text to Image。模式变化只由当前输入状态触发，不在右侧新增功能标签，也不需要用户再次选择 Auto。
  同步更新该选项在可见文本、按钮文本、移动端截断前完整值、辅助技术可访问名称和当前选中标记中的名称；不得只改下拉菜单而让关闭后的触发按钮继续显示 No tool。现有工具标识、请求参数或内部枚举可以保持不变，前端不得依赖新的显示文案作为业务逻辑值。
- 验收标准：首页集合 Workspace 和完整 Workspace 首次打开时，工具选择按钮显示 Auto；展开 Choose a tool 后第一项为 Auto、显示指定说明并具有当前选中标记，页面可见区域和可访问树中均不再出现 No tool 或 No tools。Auto 状态下分别执行纯文字任务、添加图片后的任务及移除全部图片后的任务，依次使用 Text to Image、Image to Image、Text to Image 的原有处理流程，输入、参数、每日额度与输出行为没有变化。选择其他工具后按钮显示该工具名称，再切回 Auto 后按钮恢复 Auto；桌面端、390px 和 320px 移动端无溢出或额外右侧标签，键盘和触摸均可选择。
- 不要修改：不要使用 Automatic、Smart、Default、No tool、No tools 或其他替代名称；不要把 Auto 建成新的图片处理工具、页面、接口或独立任务类型；不要改变 Text to Image、Image to Image 的实际模型、参数、质量、额度、登录或自动切换规则；不要为这项常规文案修正新增 Changelog 记录。
...
- 修改要求：为 Workspace 建立移动优先的响应式布局，不得依赖桌面固定宽度、固定高度或页面级横向滚动。仅在登录后的完整 Workspace 页面中，桌面端与移动端均删除顶部 Header 内全部功能分类菜单及其下拉入口，包括 AI Generate、AI Enhance、AI Remove、AI Edit、AI Beauty 和同类工具分类；不得在移动端汉堡菜单、更多菜单或其他 Header 入口中重复保留这些分类。Workspace Header 只保留品牌首页入口、清晰的 Workspace 页面标识，以及当前已批准的 Library 和账户相关入口；主内容占满移除菜单后释放的可用宽度。
  ...
  在所有触屏或无悬浮能力的设备上，每个已经上传、从 Library 选取或通过 Use this image 选入任务创建区的图片预览，都必须始终显示独立的移除按钮，不得依赖 `hover`、鼠标移动、长按或先选中图片才出现。按钮使用清晰的关闭或移除图标，固定在对应预览的右上角，可见图标与图片保持足够对比度，实际触控区域至少 44×44 CSS px，并具有逐张可区分的可访问名称 Remove image；多图输入按当前槽位或顺序提供如 Remove image 1、Remove image 2 的名称。按钮不得被预览裁切、底部模块滚动区域、工具栏或安全区遮挡。
  点击移除按钮只清空当前任务创建区对应的图片输入位和本地预览，不调用 Library 永久删除接口，不删除素材文件、不减少存储占用，也不影响已经提交任务所使用的输入快照。单图输入清空后恢复原上传入口；多图输入只清空所点位置，其余图片和槽位顺序保持不变。移除图片不得重置当前工具、模型、提示词或其他参数，也不得打开图片选择器、作品详情或删除确认弹窗；该操作不是永久删除，因此不要求二次确认。桌面端可以保留悬浮时增强显示的视觉效果，但使用键盘聚焦图片预览时也必须显示并能够操作同一个 Remove image 按钮。
...
- 验收标准：分别在 320×568、360×800、390×844、430×932 竖屏视口和 844×390 横屏视口中，从登录进入 Workspace 开始，能够完整完成选择工具、从本地或 Library 选图、移除当前输入图片、填写提示词、修改参数、提交任务、查看运行状态、打开作品、下载、删除和多选操作；全流程没有页面级横向滚动、内容裁切、控件重叠、不可见关闭按钮或被固定区域覆盖的最后一项。手机视口中整个任务创建模块始终固定在当前可视窗口底部，滚动作品、选择图片、展开参数、出现错误、打开和关闭软键盘后均不离开底部、不超过宽度且不遮住最后一张作品；竖屏未打开键盘时模块高度不超过 55% 的动态视口。320–767px 下作品始终每行显示两张，连续加载至少 20 张作品后仍保持等宽双列、稳定间距和平滑纵向滚动。
  在 iOS Safari、Android Chrome 及浏览器触摸模拟中，分别通过本地上传、Choose from Library 和 Use this image 添加单图与多图，不进行任何悬浮或长按即可立即看见并点击每张图片的 Remove image 控件。单图移除后恢复上传入口；多图只清除目标槽位，其他图片顺序不变；工具、模型、提示词和参数保持不变。网络记录中不得出现素材永久删除请求，Library 文件、数量和存储占用保持不变，已提交任务不受影响。320px 视口及 200% 缩放下移除按钮仍完整可见、触控区域不小于 44×44 CSS px，点击按钮不会同时打开预览或选择器；桌面键盘用户聚焦预览后也能发现并操作同一按钮。
