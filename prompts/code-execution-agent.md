...
10. 只有当网站待办文件中明确包含已批准的公开 change log 任务时，才在其他批准修改完成后新增一项记录；没有该任务就不要更新 change log。记录只总结本批次真实上线的重要用户可见功能、重大流程或导航改版、重要兼容性改进或严重问题修复。普通换图、文案或错字修正、纯 SEO 元数据调整、轻微视觉微调、内部重构和日常维护不记录；若没有重要修改成功上线，则不要发布 change log。
...
13. 如果待办中存在公开 change log 任务但 `changelog_url` 为 `not_established`，或其他元数据缺失、指令冲突、需要未提供的密钥、任务仍需决定、修改会超出批准范围，停止并向当前操作者说明阻塞，不要猜测。没有 change log 任务时，`changelog_url` 为 `not_established` 不阻塞普通任务。
...
```text
读取中央建议仓库中的 `sites/<site-id>.md`，完成其中全部 `已批准任务`，并严格遵循 delivery_method。只有文件中明确包含已批准的 change log 任务时才新增一项脱敏、面向用户的记录，并且只写真实上线的重要变化。不要执行 `待决事项`，也不要修改待办文件。
```
